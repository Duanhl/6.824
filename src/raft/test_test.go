...
func For2023TestFollowerFailure2B(t *testing.T) {
	...
	// check that command 104 did not committed.
...
func TestBackup2B(t *testing.T) {
	...
	// submit lots of commands that won't committed
	for i := 0; i < 50; i++ {
	...
	// lots more commands that won't committed
	for i := 0; i < 50; i++ {
...
func internalChurn(t *testing.T, unreliable bool) {
	...
	cfn := func(me int, ch chan []int) {
		...
		for atomic.LoadInt32(&stop) == 0 {
			...
			if ok {
				// maybe leader will committed our value, maybe not.
